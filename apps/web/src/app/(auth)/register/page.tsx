...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      result?.error ? setError('Account created but sign-in failed — try logging in') : (router.push('/dashboard'), router.refresh())
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0A0A0A] px-4">
      <div className="w-full max-w-[360px]">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          <div>
            <label className="mb-1 block text-[12px] font-medium text-[#A3A3A3]">Name</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="h-9 w-full rounded-lg border border-[#262626] bg-[#171717] px-3 text-[13px] text-[#FAFAFA] placeholder-[#737373] outline-none focus:border-[#404040]" placeholder="Your name" />
          </div>
          <div>
          ...
          <div>
            ...
            <PasswordInput value={password} onChange={setPassword} placeholder="Min 6 characters" className="h-9 rounded-lg bg-[#171717]" />
          </div>
        ...
        <p className="mt-6 text-center text-[12px] text-[#A3A3A3]">
          Already have an account? <Link href="/login" className="text-[#FAFAFA] hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  )
